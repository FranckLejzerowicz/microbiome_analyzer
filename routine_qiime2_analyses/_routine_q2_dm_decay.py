...
def get_dm_decay_config(decay_config: dict) -> (dict, dict):
...
def run_dm_decay(i_datasets_folder: str, betas: dict, p_dm_decay: str,
    ...
    job_folder2 = get_job_folder(i_datasets_folder, 'dm_decay/chunks')
    dm_decay_config = read_yaml_file(p_dm_decay)
    subsets, modes, params = get_dm_decay_config(dm_decay_config)

    all_sh_pbs = {}
    decay_res = {}
    for dat, rarefs_metrics_groups_metas_qzas_dms_trees in betas.items():
        if not split:
            out_sh = '%s/run_dm_decay_%s_%s%s.sh' % (
        ...
        for idx, metrics_groups_metas_qzas_dms_trees in enumerate(
            ...
            odir = get_analysis_folder(i_datasets_folder, 'decay/%s%s' % (
                dat, cur_raref))
            if split:
                out_sh = '%s/run_dm_decay_%s_%s%s%s.sh' % (
                    ...
                    for (meta, qza, mat_qza, tree) in metas_qzas_mat_qzas_trees:
                        ...
                        for case_var, case_vals_list in cases_dict.items():
                            for case_vals in case_vals_list:
                                case = get_case(
                                    case_vals, case_var).replace(' ', '_')
                                cur_sh = '%s/run_dm_decay_%s%s_%s_%s_%s%s.sh'\
    ...
    job_folder = get_job_folder(i_datasets_folder, 'dm_decay')
    main_sh = write_main_sh(job_folder, '3_run_dm_decay_%s%s' % (prjct_nm,
                                                                 filt_raref),
    ...
    if main_sh:
        if p_dm_decay:
            print("# dm decay (config in %s)" % p_dm_decay)
        else:
            print("# dm decay")
        ...
        for mode, mode_groups in modes.items():
            for mode_group in mode_groups:
                ...
                if not isfile(new_tsv) or force:
                    write_dm_decay(mat_qza, mat_qza_filt, new_qza, new_tsv,
