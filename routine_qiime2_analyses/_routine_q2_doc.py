...
def run_single_doc(odir: str, tsv: str, meta_pd: pd.DataFrame, case_var: str,
                   case_vals_list: list, cur_sh: str, force: bool, filt: str, fp: str,
                   fa: str, n_nodes: str, n_procs: str) -> None:
    remove = True
    qza = '%s.qza' % splitext(tsv)[0]
    with open(cur_sh, 'w') as cur_sh_o:
        for case_vals in case_vals_list:
            case = get_case(case_vals, '', case_var)
            cur_rad = '%s/%s_%s' % (odir, case.strip('_'), filt)
...
def run_doc(i_datasets_folder: str, datasets: dict, p_doc_config: str,
    ...
    doc_config, main_cases_dict = get_doc_config(p_doc_config)
    ...
    for dat, tsv_meta_pds in datasets.items():
        ...
        for case_var, case_vals_list in cases_dict.items():
            for filt, (fp, fa) in filters.items():
                cur_sh = '%s/run_doc%s_%s_%s%s_%s.sh' % (
                    job_folder2, evaluation, dat, case_var, filt_raref, filt)
                cur_sh = cur_sh.replace(' ', '-')
                all_sh_pbs.setdefault((dat, out_sh), []).append(cur_sh)
                run_single_doc(odir, tsv, meta_pd, case_var,
                               case_vals_list, cur_sh, force, filt, fp, fa,
