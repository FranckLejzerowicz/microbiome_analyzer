...
def run_procrustes(i_datasets_folder: str, datasets: dict, datasets_filt: dict,
    ...
    if not isfile(out_R):
        ...
        with open(R_script, 'w') as o:
            ...
            o.write("    f1 <- row[8]\n")
            o.write("    f2 <- row[9]\n")
            o.write("    filin_tsv_pd1 <- read.csv(f1, header = TRUE, check.names=FALSE, colClasses = 'character', row.names = 1, sep = '\\t')\n")
            o.write("    filin_tsv_pd2 <- read.csv(f2, header = TRUE, check.names=FALSE, colClasses = 'character', row.names = 1, sep = '\\t')\n")
