...
def run_procrustes(i_datasets_folder: str, datasets: dict, p_procrustes: str,
    ...
    if not isfile(out_R):
        ...
        with open(R_script, 'w') as o:
            ...
            o.write("    message(f2)\n")
            o.write("\n")
            o.write("    filin_tsv_pd1 <- read.csv(f1, header = TRUE, check.names=FALSE, row.names = 1, sep = '%s')\n" % '\t')
            o.write("    filin_tsv_pd2 <- read.csv(f2, header = TRUE, check.names=FALSE, row.names = 1, sep = '%s')\n" % '\t')
            o.write("    filin_tsv_pd1 <- as.matrix(filin_tsv_pd1)\n")
            o.write("    filin_tsv_pd2 <- as.matrix(filin_tsv_pd2)\n")
            o.write("\n")
            o.write("    cat(paste('meta dimension:', paste(as.character(dim(filin_tsv_pd1)), collapse = '-'), typeof(filin_tsv_pd1), '%s', sep=' '))\n" % '\n')
            o.write("    cat(paste('meta dimension:', paste(as.character(dim(filin_tsv_pd2)), collapse = '-'), typeof(filin_tsv_pd2), '%s', sep=' '))\n" % '\n')
