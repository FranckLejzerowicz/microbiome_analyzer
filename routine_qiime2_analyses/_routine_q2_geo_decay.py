...
def get_geo_decay_config(decay_config: dict) -> (dict, dict):
...
def run_geo_decay(i_datasets_folder: str, betas: dict, p_geo_decay: str,
    ...
    job_folder2 = get_job_folder(i_datasets_folder, 'geo_decay/chunks')
    geo_decay_config = read_yaml_file(p_geo_decay)
    subsets, modes, params = get_geo_decay_config(geo_decay_config)

    all_sh_pbs = {}
    decay_res = {}
    for dat, rarefs_metrics_groups_metas_qzas_dms_trees in betas.items():
        if not split:
            out_sh = '%s/run_geo_decay_%s_%s%s.sh' % (job_folder2, prjct_nm,
                                                   dat, filt_raref)
        decay_res[dat] = []
        for idx, metrics_groups_metas_qzas_dms_trees in enumerate(
            ...
            odir = get_analysis_folder(i_datasets_folder, 'geo_decay/%s%s' % (
                dat, cur_raref))
            if split:
                out_sh = '%s/run_geo_decay_%s_%s%s%s.sh' % (
                    ...
                    for (meta, qza, mat_qza, tree) in metas_qzas_mat_qzas_trees:
                        ...
                        for case_var, case_vals_list in cases_dict.items():
                            for case_vals in case_vals_list:
                                ...
                                cur_sh = '%s/run_geo_decay_%s%s_%s_%s_%s%s.sh' % (
                                    job_folder2, dat, cur_raref, metric,
                                    group, case, filt_raref)
                                cur_sh = cur_sh.replace(' ', '-')
                                all_sh_pbs.setdefault(
                                    (dat, out_sh), []).append(cur_sh)
    ...
    job_folder = get_job_folder(i_datasets_folder, 'geo_decay')
    main_sh = write_main_sh(job_folder, '3_run_geo_decay_%s%s' % (prjct_nm,
                                                                 filt_raref),
                            all_sh_pbs, '%s.prm%s' % (prjct_nm, filt_raref),
    ...
    if main_sh:
        if p_dm_decay:
            print("# geo decay (config in %s)" % p_dm_decay)
        else:
            print("# geo decay")
    ...
    with open(cur_sh, 'w') as cur_sh_o:
        for mode, mode_groups in modes.items():
            for mode_group in mode_groups:
                ...
                new_qza = '%s_geo_decay.qza' % cur_rad
                new_tsv = '%s_geo_decay.tsv' % cur_rad
                if not isfile(new_tsv) or force:
                    write_geo_decay(mat_qza, mat_qza_filt, new_qza, new_tsv,
...
def geo_decay_figure(i_datasets_folder: str, geo_decay_res: dict,
                     datasets_rarefs: dict, filt_raref: str) -> None:

    odir = get_analysis_folder(i_datasets_folder, 'decay')
    for dat, metrics_group_subset_mode_modegrps in geo_decay_res.items():
        not_exists_yet = 0
        decays = []
        rarefs = set()
        groups = set()
        ...
        for aitchison, decays_pd in decays_pds.items():
            ...
            fig_o = '%s/%s_geo_decays%s.pdf' % (odir, dat, aitchison)
            plt.savefig(fig_o, bbox_inches='tight')
            print('    (decay) Written figure: %s' % fig_o)
            if not_exists_yet:
                print('            -> it has %s geo decay analyses missing ('
                      'need [TO RUN])' % not_exists_yet)
