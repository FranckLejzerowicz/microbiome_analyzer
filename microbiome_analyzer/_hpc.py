        ...
        if not isdir(ret):
            ...
            if not mk or mk.lower() == 'y':
                try:
                    ...
                    raise IOError('Enter a valid path for "%s"' % scratch)
        scratch_folder = ret
...
def edit_scratch(self, logging, scratch_fp: str, scratch: str) -> str:
    """
    ...
    if self.config_scratch:
        logging.info("Registered %s: %s" % (scratch, scratch_folder))
